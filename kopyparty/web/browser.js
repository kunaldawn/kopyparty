...
if (window.dgauto === undefined)
	alert('FATAL ERROR: receiving stale data from the server; this may be due to a broken reverse-proxy (stuck cache). Try restarting kopyparty and press CTRL-SHIFT-R in the browser');
...
function langtest() {
	...
	for (var a = 1; a < LANGS.length; a++)
		import_js(SR + '/.kpr/w/tl/' + LANGS[a] + '.js', function () { if (!--n) langtest2(); });
}
function langtest2() {
...
function MPlayer() {
	...
	r.init_fau = function () {
		...
		r.fau = new Audio(SR + '/.kpr/w/deps/busy.mp3?_=' + TS);
		r.fau.loop = true;
		r.fau.play();
	};
...
var showfile = (function () {
	...
	r.setstyle = function () {
		...
		el.rel = 'stylesheet';
		el.href = SR + '/.kpr/w/deps/prism' + (light ? '' : 'd') + '.css?_=' + TS;
		...
		fetch(url, {'signal': r.abrt.signal}).then(function(rsp) {
			...
			var rf = function() {
				ro.read().then(function(v) {
					if (r.tail_id != me)
						return;
					var vt = v.done ? '\n*** lost connection to kopyparty ***' : v.value;
					if (vt == '\x00')
						return rf();
					txt += vt;
		...
		if (is_md) {
			...
			if (!window.no_prism && !skip_prism) {
				...
				if (!defer)
					...
					import_js(SR + '/.kpr/w/deps/prism.js', function () { fun(); });
			}
			if (!txt && r.wrap)
				el.className = 'wrap';
		}
		...
		for (var a = 0, aa = files.length; a < aa; a++) {
			...
			if (use_ext_th && (ext_th[ext] || ext_th[ext0])) {
				...
				if (r.x3)
					ihref += '3';
				if (href == "#")
					ihref = SR + '/.kpr/ico/' + (ref == 'moar' ? '++' : 'exit');
			}
			else if (isdir) {
				ihref = SR + '/.kpr/ico/folder';
			}
			else {
				if (!svgs.has(ext)) {
...
function show_md(md, name, div, url, depth) {
	...
	if (!marked) {
		...
		wfp_debounce.n--;
		return import_js(SR + '/.kpr/w/deps/marked.js', function () {
