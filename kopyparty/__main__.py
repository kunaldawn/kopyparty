...
"""kopyparty: http file sharing hub (py2/py3)"""
...
def init_E(EE: EnvParams) -> None:
    ...
    def get_unixdir() -> tuple[str, bool]:
        ...
        for npath, (pf, pa) in enumerate(paths):
            ...
            try:
                ...
                if ram:
                    t = "Using %s/kopyparty [%s] for config; filekeys/dirkeys will change on every restart. Consider setting XDG_CONFIG_HOME or giving the unix-user a ~/.config/"
                    errs.append(t % (pa, p))
                elif mkdir:
                    t = "Using %s/kopyparty [%s] for config%s (Warning: %s did not exist and was created just now)"
                    errs.append(t % (pa, p, " instead" if npath else "", pa))
                elif errs:
                    errs.append("Using %s/kopyparty [%s] instead" % (pa, p))
    ...
    if p:
        ...
        E.cfg = os.path.normpath(bdir + "/kopyparty")
    elif sys.platform == "darwin":
        E.cfg = os.path.expanduser("~/Library/Preferences/kopyparty")
    else:
...
def ensure_webdeps() -> None:
  ...
  however, if you are a dev, or running kopyparty from source, and you want
...
def get_sects():
    return [
        [
            ...
            (usually \033[33m0600\033[0m) and the same user/group as kopyparty

            \033[32m-i fd:\033[33m3\033[0m uses the socket passed to kopyparty on file descriptor 3
        ...
        [
            ...
            dedent(
                """
            usually kopyparty returns a \033[33m404\033[0m if a file does not exist, and
            ...
            the file must define the function \033[35mmain(cli,vn,rem)\033[0m:
             \033[35mcli\033[0m: the kopyparty HttpCli instance
            ...
            > \033[32m"retry"\033[0m: the plugin has done something to resolve the 404
                situation, and kopyparty should reattempt reading the file.
            ...
            \033[1;35mPS!\033[0m the folder that contains the python file should ideally
              ...
              with filenames that overlap with modules used by kopyparty
            """
            ),
        ],
        [
            ...
            optionally prefix the command with comma-sep. flags similar to -mtp:
             ...
             \033[36mkn\033[35m lets it continue running until kopyparty is terminated
            ...
            examples:
             ...
             \033[36m--xm aw,,notify-send,hey,--\033[35m shows an OS alert on linux;
              the \033[33m,,\033[35m stops kopyparty from reading the rest as flags and
            ...
            then able to tamper with kopyparty's internal memory
        ...
        [
            ...
            integrate kopyparty with that for automatic login
        ...
        [
            ...
            kopyparty will also hash and print any passwords that are non-hashed
        ...
        [
            ...
            * is there a firewall blocking port 5353 on either the server or client?
              (for example, clients may be able to send queries to kopyparty,
            ...
            * is there a firewall blocking port 5353 on either the server or client?
              (kopyparty may be unable to see the queries from the clients, but the
            ...
            * does the client have multiple IPs on its interface, and some of the
              IPs are in subnets which the kopyparty server is not a member of?
...
def add_network(ap):
    ...
    if ANYWIN:
        ap2.add_argument("--reuseaddr", action="store_true", help="set reuseaddr on listening sockets on windows; allows rapid restart of kopyparty at the expense of being able to accidentally start multiple instances")
    elif not MACOS:
...
def add_auth(ap):
    ...
    ap2.add_argument("--idp-h-usr", metavar="HN", type=u, action="append", help="\033[34mREPEATABLE:\033[0m bypass the kopyparty authentication checks if the request-header \033[33mHN\033[0m contains a username to associate the request with (for use with authentik/oauth/...)\n\033[1;31mWARNING:\033[0m if you enable this, make sure clients are unable to specify this header themselves; must be washed away and replaced by a reverse-proxy")
    ap2.add_argument("--idp-hm-usr", metavar="T", type=u, action="append", help="\033[34mREPEATABLE:\033[0m bypass the kopyparty authentication checks if the request-header \033[33mT\033[0m is provided, and its value exists in a mapping defined by this option; see --help-idp")
    ...
    ap2.add_argument("--idp-db", metavar="PATH", type=u, default=idp_db, help="where to store the known IdP users/groups (if you run multiple kopyparty instances, make sure they use different DBs)")
    ...
    ap2.add_argument("--ses-db", metavar="PATH", type=u, default=ses_db, help="where to store the sessions database (if you run multiple kopyparty instances, make sure they use different DBs)")
...
def add_chpw(ap):
    ...
    ap2.add_argument("--chpw-db", metavar="PATH", type=u, default=db_path, help="where to store the passwords database (if you run multiple kopyparty instances, make sure they use different DBs)")
...
def add_zc_mdns(ap):
    ...
    ap2.add_argument("--zm-noneg", action="store_true", help="disable NSEC replies -- try this if some clients don't see kopyparty")
...
def add_smb(ap):
    ...
    ap2.add_argument("--smb", action="store_true", help="enable smb (read-only) -- this requires running kopyparty as root on linux and macos unless \033[33m--smb-port\033[0m is set above 1024 and your OS does port-forwarding from 445 to that.\n\033[1;31mWARNING:\033[0m this protocol is DANGEROUS and buggy! Never expose to the internet!")
...
def add_stats(ap):
    ...
    ap2.add_argument("--stats", action="store_true", help="enable openmetrics at /.kpr/metrics for admin accounts")
    ap2.add_argument("--stats-u", metavar="U,U", type=u, default="", help="comma-separated list of users allowed to access /.kpr/metrics even if they aren't admin")
...
def add_optouts(ap):
    ...
    ap2.add_argument("-nb", action="store_true", help="no powered-by-kopyparty branding in UI")
...
def add_logging(ap):
    ...
    ap2.add_argument("--lf-url", metavar="RE", type=u, default=r"^/\.kpr/|[?&]th=[xwjp]|/\.(_|ql_|DS_Store$|localized$)", help="dont log URLs matching regex \033[33mRE\033[0m")
...
def add_db_general(ap, hcores):
    ...
    ap2.add_argument("-e2vp", action="store_true", help="on hash mismatch: panic and quit kopyparty")
...
def add_og(ap):
    ...
    ap2.add_argument("--og-tpl", metavar="PATH", type=u, default="", help="do not return the regular kopyparty html, but instead load the jinja2 template at \033[33mPATH\033[0m (if path contains 'EXT' then EXT will be replaced with the requested file's extension) (volflag=og_tpl)")
...
def add_ui(ap, retry: int):
    ...
    ap2.add_argument("--doctitle", metavar="TXT", type=u, default="kopyparty @ --name", help="title / service-name to show in html documents")
...
def add_debug(ap):
    ...
    ap2.add_argument("--bf-dir", metavar="PATH", type=u, default="bf", help="bak-flips: store corrupted chunks at \033[33mPATH\033[0m; default: folder named 'bf' wherever kopyparty was started")
...
def main(argv: Optional[list[str]] = None) -> None:
    ...
    f = '\033[36mkopyparty v{} "\033[35m{}\033[36m" ({})\n{}\033[0;36m\n   sqlite {} | jinja {} | pyftpd {} | tftp {} | miko {}\n\033[0m'
    f = f.format(
        S_VERSION,
        CODENAME,
        S_BUILD_DT,
