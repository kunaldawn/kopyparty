...
# KD fork: mdns / ssdp / smbd / ftpd / sftpd / tftpd type-checking imports removed
...
class SvcHub(object):
    ...
    def __init__(
        ...
        if args.vc_url:
            ...
            if zi < 3 and "api.kopyparty.eu" in args.vc_url:
                zi = 3
                self.log("root", "vc-age too low for kopyparty.eu; will use 3 hours")
            args.vc_age = zi
        ...
        if not args.no_thumb:
            ...
            if self.args.th_dec:
                ...
                if EXE:
                    msg = "kopyparty.exe cannot use Pillow or pyvips; need ffprobe.exe and ffmpeg.exe to create thumbnails"
        ...
        # KD fork: ftp/sftp/tftp/smb/mdns/ssdp protocol launchers removed.
        # this fork serves only HTTP for read-only browsing.
        if not args.zms:
            args.zms = zms
        ...
        for tries in range(2):
            ...
            try:
                ...
                try:
                    ...
                    if sver > native_ver:
                        zs = "this version of kopyparty only understands %s v%d and older; the db is v%d"
    ...
    def _verify_db(
        ...
        if not lock_file(db_path + ".lock"):
            t = "the %s [%s] is already in use by another kopyparty instance (pid:%d). This is not supported; please provide another database with --%s or give this kopyparty-instance its entirely separate config-folder by setting another path in the XDG_CONFIG_HOME env-var. You can also disable this safeguard by setting env-var PRTY_NO_DB_LOCK=1. %s"
    ...
    def setup_share_db(self) -> None:
        ...
        try:
            ...
            if not create:
                ...
                if sver > VER_SHARES_DB:
                    zs = "this version of kopyparty only understands shares-db v%d and older; the db is v%d"
                    raise Exception(zs % (VER_SHARES_DB, sver))

                cur.execute("select count(*) from sh").fetchone()
        except Exception as ex:
            t = "could not open shares-db; will now panic...\nthe following database must be repaired or deleted before you can launch kopyparty:\n%s\n\nERROR: %s\n\nadditional details:\n%s\n"
        ...
        if not lock_file(db_lock):
            t = "the shares-db [%s] is already in use by another kopyparty instance (pid:%d). This is not supported; please provide another database with --shr-db or give this kopyparty-instance its entirely separate config-folder by setting another path in the XDG_CONFIG_HOME env-var. You can also disable this safeguard by setting env-var PRTY_NO_DB_LOCK=1. Will now panic."
            t = t % (db_path, owner)
            self.log("root", t, 1)
            raise Exception(t)
    ...
    # KD fork: start_ftpd / restart_sftpd / restart_ftpd / restart_tftpd removed
    # (protocol modules deleted; this fork serves HTTP only)
    ...
    # KD fork: sticky_qr / _unsticky_qr / _sticky_qr / _qr_thr removed
    # (terminal QR-code feature dropped along with qrkode module)
    ...
    def cb_httpsrv_up(self) -> None:
        ...
        # KD fork: FTP startup wait + terminal QR-code printing removed
    ...
    def start_zeroconf(self) -> None:
        # KD fork: mdns / ssdp zeroconf support removed
        self.zc_ngen += 1
    ...
    def reload(self, rescan_all_vols: bool, up2k: bool) -> str:
        ...
        with self.reload_mutex:
            ...
            t += "\n\nchanges to global options (if any) require a restart of kopyparty to take effect"
            self.broker.reload()
            self._reload_thumbsrv()
        return t
    ...
    def shutdown(self) -> None:
        ...
        try:
            ...
            tasks = []
            slp = 0.0
            # KD fork: mdns / ssdp / smbd shutdown removed
            ...
            try:
                ...
                for adv in advisories:
                    ...
                    for x in adv["vulnerabilities"]:
                        if x["package"]["name"].lower() == "kopyparty":
                            vuln = x
                            break
                    if not vuln:
                        continue
