...
## so one popular way to avoid issues is by running kopyparty without users;
...
## obviously does not exist and will never be able to access any kopyparty
...
class VFS(object):
    ...
    def walk(
        ...
        # if lstat: ignore folder symlinks since kopyparty will never make those
...
class AuthSrv(object):
    ...
    def _reload(self, verbosity: int = 9) -> None:
        ...
        if have_fk and fk_len < 14:
            t = "WARNING: filekeys are enabled, but the salt is only %d chars long; %d or longer is recommended. Either specify a stronger salt using --fk-salt or delete this file and restart kopyparty: %s"
